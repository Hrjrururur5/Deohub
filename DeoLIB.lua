...
local WHITELIST_URL = nil  -- set via SetKeySystem if needed
local KEY_FORMULA   = nil
local KEY_FILE      = "deolib_key.txt"
local PORTAL_URL    = "https://file.garden/ahkRbB3bekZ7SJ00/whitelist.html"
...
task.spawn(function()
    ...
    keySubtitle.Text                 = PORTAL_URL and ("Get your key at the Deo Hub portal") or "Enter your key below to continue"
    ...
    -- Get Link button: opens the portal in the Roblox browser
    local getLinkBtn = Instance.new("TextButton")
    getLinkBtn.Size                 = UDim2.new(1, -48, 0, 36)
    getLinkBtn.Position             = UDim2.new(0, 24, 0, 260)
    getLinkBtn.BackgroundColor3     = Color3.fromRGB(22, 22, 30)
    getLinkBtn.BorderSizePixel      = 0
    getLinkBtn.Text                 = "🔗  Get Link"
    getLinkBtn.TextColor3           = Color3.fromRGB(180, 180, 255)
    getLinkBtn.TextSize             = 13
    getLinkBtn.Font                 = Enum.Font.GothamSemibold
    getLinkBtn.ZIndex               = 7
    getLinkBtn.Parent               = keyPage
    Instance.new("UICorner", getLinkBtn).CornerRadius = UDim.new(0, 8)
    local getLinkStroke = Instance.new("UIStroke")
    getLinkStroke.Color       = Color3.fromRGB(140, 140, 255)
    getLinkStroke.Transparency = 0.65
    getLinkStroke.Thickness    = 1
    getLinkStroke.Parent       = getLinkBtn

    getLinkBtn.MouseButton1Click:Connect(function()
        nativeClick()
        getLinkBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 60)
        task.delay(0.15, function()
            if getLinkBtn and getLinkBtn.Parent then
                getLinkBtn.BackgroundColor3 = Color3.fromRGB(22, 22, 30)
            end
        end)
        local portalUrl = PORTAL_URL and (PORTAL_URL .. "?HWID=" .. userId) or nil
        if portalUrl then
            pcall(function() setclipboard(portalUrl) end)
            -- Also try to open directly in Roblox browser
            pcall(function()
                game:GetService("GuiService"):OpenBrowserWindow(portalUrl)
            end)
            Notify("Deo Hub", "Portal opened! If it didn't open, link copied — paste in browser.", 5)
        else
            Notify("Deo Hub", "No portal URL configured.", 3)
        end
    end)

    local keyDivider = Instance.new("Frame")
    keyDivider.Size                 = UDim2.new(1, -48, 0, 1)
    keyDivider.Position             = UDim2.new(0, 24, 0, 308)
    ...
    checkingLabel.Position             = UDim2.new(0, 16, 0, 318)
    ...
    local API_URL = PORTAL_URL and (PORTAL_URL .. "?HWID=") or nil

    local function checkKeylessBackend()
        if not API_URL then return false end
        -- Check localStorage state via a netlify function if available,
        -- otherwise fall back to formula/whitelist checks
        local ok, result = pcall(function()
